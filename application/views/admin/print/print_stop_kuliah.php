...
<div class="container-fluid">
    ...
    <center>
        <table class="kop mb-5">
            <td style="font-size: 30px; font-weight: bold;">Laporan Data Pengajuan Surat Bebas Perpustakaan (Berhenti Kuliah)</td>
        </table>
    </center>
    ...
    <div>
        <table class="isi" style="width:100%;">
            ...
            <tbody style="line-height: 30px;">
                <?php $no = 1;
                foreach ($print_stop_kuliah as $s) : ?>
                    <tr>
                        <td><?php echo $no++ ?></td>
                        <?php if ($s->nomor_surat == null) { ?>
                            <td>-</td>
                        <?php } else { ?>
                            <td>Nomor: <?php echo $s->nomor_surat ?>/PL40.14/TU/<?php echo date("Y", strtotime($s->tanggal_surat)); ?></td>
                        <?php } ?>
